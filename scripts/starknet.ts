...
export async function starknetDeclare(networkName: string) {
  ...
  const matchErr = err.match(
    /Class with hash (0x[0-9a-fA-F]+) is already declared./,
  );
...
export async function starknetDeploy(
  ...
  const result =
    await $`sncast deploy --class-hash ${classHash} --network ${STARKNET_CHAIN_IDS[chainId]} --constructor-calldata ${calldata}`
      .env({
        ...process.env,
        FORCE_COLOR: "1",
      })
...
export async function starknetGetAccount(
  networkName: (typeof STARKNET_CHAIN_IDS)[keyof typeof STARKNET_CHAIN_IDS],
) {
  let account: string;
  try {
    const accounts = fs
      .readFileSync(import.meta.dirname + "/../cairo/.env", "utf8")
      .split("\n")
  ...
  const path = accountList
    .split("\n")[0]
    ?.match(/Available accounts \(at (.+)\):/)?.[1];
  if (path === undefined) {
...
export async function starknetUpgrade(
  network: (typeof STARKNET_CHAIN_IDS)[keyof typeof STARKNET_CHAIN_IDS],
  satelliteAddress: string,
  classHash: string,
) {
  // TODO: not tested
  await $`sncast invoke --contract-address ${satelliteAddress} --function upgrade --calldata "${classHash}" --network ${network}`
    .env({
      ...process.env,
      FORCE_COLOR: "1",
    })
